...
Another example with suggestions to implement the functional complex network

- Add a new section (and most likely a new function) where we use the dictionnaries containing the location of the stands (`standCoordinatesDict`), the one containing the neighbor of each stand (`standNeighboursDict`), and the one containing the composition of each stand (`standCompositionDict`) to create a functional complex network. We can compute the functional diversity of each stand with their composition (`standCompositionDict`), and then make a network with the position of the stands (`standCoordinatesDict` and `standNeighboursDict`)

- We can then write a new algorithm of selection of prescription for each stand, etc.
