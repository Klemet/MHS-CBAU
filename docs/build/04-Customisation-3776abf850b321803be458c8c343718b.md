...
Then, you just have to enable debug mode in the main MHS-CBAU script (`MHS-CBAU_MainScript.py`) by turning `DEBUG_MODE_ENABLED` to `True`, and filling out the line with `os.chdir` to point to the folder where the output files are.
