...
The scripts will output two files you need to keep at hand : `EcozonesRaster.tif`, and `merchantableBiomassRatiosDictionnary.json`. BIG WARNING HERE : let’s differenciate ecozones and ecoregions. Ecozones are a classification system used by the different canadian agencies. They are like “Thundra, Mixed forest, etc.”. Don’t confuse them with ecoregions, which is a term used in LANDIS-II to define, inside the model, regions or pixels where the physical conditions (climate, slope, aspect, soils, etc.) are considered homogenous. So here, I’m talking about ecozones, the system used by the Canadian government – because the ratios we get here are defined per province and per ecozone. 

So, as I said, the script will output two files you need to keep at hand : `EcozonesRaster.tif`, and `merchantableBiomassRatiosDictionnary.json`. The ecozone raster contains a special ecozone ID representing a combo of ecozone/province so that we can get the right ratio for your species depending on where it is in Canada. The ecozone raster is made to have the same size as the other raster maps of your LANDIS-II simulation to make things easier. Again, don’t confuse it with your “Ecoregion raster”, which is a raster needed by LANDIS-II no matter what. I’m sorry that the terms are confusing.

One last thing : as climate change comes, the position of the ecozones of Canada (as defined by the canadian government) might change with time. Deciduous forests will climb up north, etc. However, we don’t deal with this change in position in MHS-CBAU. The ecozone rasters created here is made using the official files showing the position of ecozones in Canada today. Apparently, while we know that ecozones will change in the future because of climate change, we don’t really have models to have predictions of their evolution. As such, we keep things static in MHS-CBAU. This might be a limitation that you might want to highlight in your papers if you use MHS-CBAU; although it will only concern these biomass ratios we used to compute merchantable volume from aboveground biomass.
